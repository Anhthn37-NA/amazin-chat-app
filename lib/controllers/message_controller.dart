...
class MessageController extends ChangeNotifier{
  ...
  MessageController({required this.user, this.receiver, this.chatRoomId}) {
    _init();
  }
  ...
  Future _init() async {
    if (chatRoomId == null && receiver != null) {
      chatRoomId = await firebaseServices.getIdRoomChat(user.uid!, receiver!.uid!);
    }

    //Bắt đầu lắng nghe messages trong phòng chat
    if (chatRoomId != null) {
      listenMessages();
    }
  ...
  Future mediaPicker(BuildContext context, RequestType requestType) async {
    ...
    if (result.isAuth) {
      ...
      for (var media in mediaList) {
        final data = await media.thumbnailDataWithSize(const ThumbnailSize(300, 300));
        mediaData.add(data!);
      }
      notifyListeners();
  ...
  void listenMessages() {
    ...
    firebaseServices.getStreamMessage(chatRoomId!).listen((snapshot) {
      listmessages = snapshot.docs
          .map((doc) => Message.fromJson(doc.data()))
          .toList();
      notifyListeners(); // Đảm bảo rebuild lại UI khi có tin nhắn mới
    });
  }
