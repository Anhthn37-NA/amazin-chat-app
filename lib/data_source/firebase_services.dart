...
import 'package:achat/data_source/firebase_repository.dart';
...
class FirebaseService {
      ...
      await firebaseRepository.firebaseAuth
        ...
        if (value.user != null) {
          ...
          await createUserToDatabase(
            appUser,
          ).then((value) => onDone!(appUser));
        }
      });
      Utils.showToast(SUCCESS_REGISTER); //"Account created successfully"
  ...
  Future signInWithGoogle() async {
    UserCredential? user;
    try {
      GoogleSignInAccount? signInAccount = await firebaseRepository.googleSignIn
          .signIn();
      if (signInAccount != null) {
        ...
        user = await firebaseRepository.firebaseAuth.signInWithCredential(
          credential,
        );
  ...
  Future<AppUser?> getUser(String uid) async {
    final snapshot = await firebaseRepository.firestore.collection("users").doc(
        uid).get();
    if (snapshot.exists) {
  ...
  Future sendMessage(
    ...
    await firebaseRepository.firestore
        .collection("chats")
        .doc(roomChat.id)
        .set(roomChat.toJson(), SetOptions(merge: true));
  }
  ...
  Future<String?> getIdRoomChat(String senderId, String recieverId) async {
    final snapshot = await firebaseRepository.firestore
        .collection("chats")
        .get();
  ...
  Future<String?> uploadFileMessage(String userId, File file) async {
    ...
    try {
      final uploaTask =
      await firebaseRepository.firebaseStorage
  ...
  Future updateMessageMedias(List<String> medias, String roomChatId,
